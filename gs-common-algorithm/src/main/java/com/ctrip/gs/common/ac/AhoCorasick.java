...
/**
 * <p>
 * An implementation of the Aho-Corasick string searching automaton. This implementation of the <a
 * href="http://portal.acm.org/citation.cfm?id=360855&dl=ACM&coll=GUIDE" target="_blank">Aho-Corasick</a> algorithm is
 * optimized to work with bytes.
 * </p>
 * 
 * <p>
 ...
 * <ul>
 * 
 * <li>Per user request from Carsten Kruege, I've changed the signature of State.getOutputs() and
 * SearchResults.getOutputs() to Sets rather than Lists.</li>
 * 
 * </ul>
 */
public class AhoCorasick {
    ...
    /**
     * Adds a new keyword with the given output. During search, if the keyword is matched, output will be one of the
     * yielded elements in SearchResults.getOutputs().
     */
    ...
    /**
     * DANGER DANGER: dense algorithm code ahead. Very order dependent. Initializes the fail transitions of all states
     * except for the root.
     */
    ...
    /**
     * Returns the root of the tree. Package protected, since the user probably shouldn't touch this.
     */
    State getRoot() {
        return this.root;
    }
     ...
     */
    SearchResult startSearch(char[] bytes) {
        if (!this.prepared) {
    ...
    /**
     * Continues the search, given the initial state described by the lastResult. Package protected.
     */
    SearchResult continueSearch(SearchResult lastResult) {
        char[] bytes = lastResult.bytes;
